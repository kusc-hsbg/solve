...
public class _1017 {
    ...
    static boolean[] is_prime = new boolean[MAXN + 1];
    static int arr_size;
    static List<Integer> a_set = new ArrayList<>();
    static List<Integer> b_set = new ArrayList<>();
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            for (int i = 0; i < a_set.size(); i++) {
                for (int j = 0; j < b_set.size(); j++) {
                    if (is_prime[a_set.get(i) + b_set.get(j)]) {
                        adj.get(i).add(j);
                    }
                }
            }
